...
import { Cpu } from 'lucide-react'; // Import the icon
...
export function KnowledgebaseForm() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await onSubmit(form.getValues() as KnowledgebaseValues); // Get form values and submit
    } finally {
  ...
  return (
    <div className="container mx-auto py-10">
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-1">
        <SettingCard
          icon={<Cpu className="h-8 w-8 text-primary" />} // Add an icon
          title="Cohere Top N"
